  ...
  return (
    <>
      ...
      <div className="container " >
        ...
        <strong >{char}</strong> <p style={{ display: "inline" ,color:mode.color}}>{char === 0 || char === 1 ? 'Character' : "Characters"}</p>&nbsp;And     <strong> {getWordCount(text)}</strong> <p style={{ display: "inline" }}> {getWordCount(text) === 0 || getWordCount(text) === 1 ? 'Word' : 'Words'} are found </p>  <strong>{char * 0.008} </strong> Minutes Required To read
